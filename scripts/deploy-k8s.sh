...
kubectl wait --for=condition=ready pod -l app=projecthub -n ${NAMESPACE} --timeout=300s
...
SERVICE_TYPE=$(kubectl get service projecthub-service -n ${NAMESPACE} -o jsonpath='{.spec.type}')

if [ "$SERVICE_TYPE" == "LoadBalancer" ]; then
    echo -e "\n${YELLOW}Waiting for LoadBalancer IP...${NC}"
    sleep 5
    EXTERNAL_IP=$(kubectl get service projecthub-service -n ${NAMESPACE} -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
    
    if [ -z "$EXTERNAL_IP" ] || [ "$EXTERNAL_IP" == "null" ]; then
        EXTERNAL_IP=$(kubectl get service projecthub-service -n ${NAMESPACE} -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
    fi
    ...
    if [ -n "$EXTERNAL_IP" ] && [ "$EXTERNAL_IP" != "null" ]; then
        ...
        echo -e "${BLUE}kubectl get service projecthub-service -n ${NAMESPACE}${NC}"
    fi
elif [ "$SERVICE_TYPE" == "NodePort" ]; then
    NODE_PORT=$(kubectl get service projecthub-service -n ${NAMESPACE} -o jsonpath='{.spec.ports[0].nodePort}')
...
echo -e "  View logs: ${GREEN}kubectl logs -f -l app=projecthub -n ${NAMESPACE}${NC}"
echo -e "  Scale app: ${GREEN}kubectl scale deployment projecthub-app --replicas=3 -n ${NAMESPACE}${NC}"
